...
mapper = umap.UMAP(densmap=True, dens_lambda=0,
                   n_neighbors=10, min_dist=0).fit(data)
...
# find distance between image # (idx) and all other images
# sort list to find closest n images, and display

idx = 3704
elems = len(mapper.embedding_)
darr = np.zeros((elems), dtype=np.float32)  # items,parts of item
p = mapper.embedding_[idx]
for i in range(elems):
    d = mapper.embedding_[i]
    darr[i] = jdist(p, d)
...
fout = "Select20-carsB.csv"
with open(fout, 'w') as f:
    f.write("index,dist,fname\n")
    for i in sd.iloc[0:n, 0].to_numpy():  # array of closest indices
        f.write("%6d,%6.3f,%s\n" % (i, darr[i], pname[i]))

showPlot.p20()  # display set of images
